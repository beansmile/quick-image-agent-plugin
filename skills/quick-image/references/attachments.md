...
- 附件默认是供 Quick Image 服务处理的不透明输入。除非用户明确要求内容分析，否则 Agent 不得打开、预览、播放、OCR、转录、抽帧或以其他方式读取图片、音频、视频的语义内容。
- 允许使用文件名、媒体类型、字节大小和为格式/时长/尺寸限制所必需的技术元数据；这些元数据不等同于内容分析。附件角色、画面内容和生成意图只能来自用户明确文字，不能从媒体内容推断。

## 发现与登记

- 为每个任务所需附件取得一次性 `attachment_handle`。检查仅读取文件身份、基础信息、格式及完成限制校验所需的技术元数据，计算校验和并保存轻量引用记录；不进行语义内容分析，不复制附件字节、不压缩、不暂存、不上传。
...
- OpenClaw 只有在用户有明确生成意图且任务需要附件时，才调用 `quick_image_list_attachments`；不要仅因收到附件就执行检查。不传参数时返回当前会话最近 10 个候选并按上传时间从旧到新排列，单页 `limit` 最大 20；返回 `has_more=true` 时，将 `next_cursor` 原样作为下一次调用的 `cursor`，按需继续读取更早候选。根据用户表述、数量、对话顺序以及 `message_id`、`position` 和 `received_at` 选择附件，仅对选中的 `attachment_id` 调用 `quick_image_inspect_attachment`。
...
- OpenClaw 附件发现索引在源文件仍可读取期间保留，每个 session 最多保留最近 500 条；宿主按 `media.ttlHours` 清理源文件后，索引会在下次列表或定时清理时移除。保留检查结果的 `attachment_handle`、媒体类型、大小、元数据和过期时间；本次任务中已有未过期句柄时直接复用，不重复检查。
