...
- 发送生成结果时，图片预览由本地 Runtime 受约束下载（仅 HTTPS、拒绝重定向、私网解析防护、超时与大小上限、格式校验）到私有缓存目录后，以本地文件投递给聊天渠道，并按本地检测出的格式标注带扩展名的文件名；下载或本地发送失败时退为发送原图链接文本，不回退远程 URL 投递。视频预览直接使用结果地址投递，并用任务结果返回的 `preview_content_type` 标注扩展名文件名。
