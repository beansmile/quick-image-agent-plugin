...
- 远程 MCP 提供授权撤销工具 `revoke_authorization`（退出登录）：按当前宿主种类撤销该账号的全部服务端授权，是恢复通道类工具，不受最低插件版本门禁限制；服务端撤销即为权威退出，宿主本地残留的失效凭据无害，插件不做本地清理。

共享 Skill 的 `SKILL.md` 只保留核心生命周期、安全不变式和阶段路由；鉴权、退出登录、附件、报价提交、结果轮询分别位于 `skills/quick-image/references/`，仅在进入对应阶段时读取，以减少不相关任务的上下文消耗。
